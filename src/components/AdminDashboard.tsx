...
export function AdminDashboard({ onHome, onServicesChanged }: Props) {
  ...
  const [tab, setTab] = useState<'appointments' | 'services' | 'blogs' | 'doctors' | 'popups'>('appointments')
...
const [popups, setPopups] = useState<Popup[]>([]); const [popupLoading, setPopupLoading] = useState(false); const [popupMessage, setPopupMessage] = useState(''); const [popupTitle, setPopupTitle] = useState(''); const [popupActive, setPopupActive] = useState(true); const [popupPhoto, setPopupPhoto] = useState<File | null>(null)
  ...
  const loadPopups = async () => {
    ...
    const { data, error: fetchError } = await supabase.from('popups').select('*').order('sort', { ascending: true }).order('created_at', { ascending: true })
    setPopupLoading(false)
    if (fetchError) { setPopupMessage(fetchError.code === '42P01' ? 'popups 테이블이 없습니다. 스키마를 먼저 적용해주세요.' : `팝업 목록을 불러오지 못했습니다: ${fetchError.message}`); return }
    setPopups((data ?? []) as Popup[])
  }

  const uploadPopupImage = async (file: File): Promise<string | null> => {
    if (!supabase) return null
    const ext = (file.name.split('.').pop() || 'jpg').toLowerCase()
    const path = `popup-${Date.now()}-${Math.round(Math.random() * 1000)}.${ext}`
    const { error: uploadError } = await supabase.storage.from('service-images').upload(path, file, { upsert: false })
    if (uploadError) { setPopupMessage(`사진 업로드에 실패했습니다: ${uploadError.message}`); return null }
    return path
  }
  const deletePopupImage = async (path: string) => { if (!supabase || !path) return; const { error } = await supabase.storage.from('service-images').remove([path]); if (error) setPopupMessage(`사진 삭제에 실패했습니다: ${error.message}`) }
  const addPopup = async () => {
    if (!supabase) return
    if (!popupPhoto) { setPopupMessage('팝업에 사용할 이미지를 선택해주세요.'); return }
    const image = await uploadPopupImage(popupPhoto)
    if (!image) return
    const { error: insertError } = await supabase.from('popups').insert({ title: popupTitle.trim(), image, active: popupActive, sort: Math.round(Date.now() / 1000) })
    if (insertError) { setPopupMessage(`팝업 등록에 실패했습니다: ${insertError.message}`); return }
    setPopupMessage('팝업이 등록되었습니다.')
    setPopupTitle(''); setPopupPhoto(null); setPopupActive(true)
    void loadPopups()
  }
  const togglePopupActive = async (popup: Popup) => {
    if (!supabase) return
    const { error: updateError } = await supabase.from('popups').update({ active: !popup.active }).eq('id', popup.id)
    if (updateError) { setPopupMessage(`업데이트에 실패했습니다: ${updateError.message}`); return }
    void loadPopups()
  }
  const removePopup = async (popup: Popup) => {
    if (!supabase) return
    const { error: deleteError } = await supabase.from('popups').delete().eq('id', popup.id)
    if (deleteError) { setPopupMessage(`팝업 삭제에 실패했습니다: ${deleteError.message}`); return }
    if (popup.image) await deletePopupImage(popup.image)
    setPopupMessage('팝업이 삭제되었습니다.')
    void loadPopups()
  }
